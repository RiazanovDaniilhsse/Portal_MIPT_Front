...
export default function Catalog() {
  ...
  const load = useCallback(() => {
    ...
    p.then(data => {
      ...
      // Нормализуем данные: исправляем категорию и гарантируем наличие нужных полей
      const normalizedAds = rawAds.map(ad => {
        // 1. Сначала меняем "_" на "/"
        let catName = ad.category?.includes('_')
            ? ad.category.replace(/_/g, '/')
            : (ad.category || '');

        const formattedCategory = catName
            .split('/')
            .map(word => {
  ...
  const allGroups = categories
      ...
      .map(g => ({
        name: g.groupName,
        cats: g.categories.filter(c => !type || c.type === type)
      }));
  ...
  return (
      <div style={{
        ...
        <div style={{display: 'flex', flex: 1, overflow: 'hidden'}}>
          ...
          <aside style={{
            ...
            <div style={{
              ...
              {[['', 'Все'], ['OBJECTS', 'Товары'], ['SERVICES', 'Услуги']].map(
                  ([val, label]) => (
                      <button
                          key={val}
                          onClick={() => {
                          ...
                          style={{
                            ...
                            background: type === val ? 'var(--accent-soft)'
                                : 'none',
                            color: type === val ? 'var(--accent)'
                                : 'var(--text)',
                            border: 'none',
                            borderRadius: 'var(--radius-sm)',
                            textAlign: 'left',
                    ...
                    {g.cats.map(c => (
                        <button
                            ...
                            style={{
                              ...
                              background: category === c.name
                                  ? 'var(--accent-soft)' : 'none',
                              color: category === c.name ? 'var(--accent)'
                                  : 'var(--text)',
                              border: 'none',
                              borderRadius: 'var(--radius-sm)',
                              textAlign: 'left',
                ...
                <div style={{
                  ...
                  {ads.map(ad => (
                      <AdCard
                          key={ad.id}
                          ad={ad}
                          onFavoriteToggle={updated => setAds(prev => prev.map(
        ...
        <CreateAdModal isOpen={modalOpen} onClose={() => setModalOpen(false)}
                       onSuccess={load}/>
      </div>
  );
}
